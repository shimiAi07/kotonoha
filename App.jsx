...
function _saveSession(user) {
  try { sessionStorage.setItem("fb_user", JSON.stringify(user)); } catch {}
}
function _loadSession() {
  try { const s = sessionStorage.getItem("fb_user"); return s ? JSON.parse(s) : null; } catch { return null; }
}
function _clearSession() {
  try { sessionStorage.removeItem("fb_user"); } catch {}
}
...
async function loadData() {
  try {
    const [r4, r3] = await Promise.allSettled([
      window.storage.get("kotonoha-v4"),
      window.storage.get("kotonoha-v3"),
    ]);
...
async function saveData(data) {
  try { await window.storage.set("kotonoha-v4", JSON.stringify(data)); } catch {}
}
...
function ProfilePage({ diaries, t, user }) {
  ...
  useEffect(()=>{
    window.storage.get("kotonoha-profile").then(r=>{ if(r) setProfile(JSON.parse(r.value)); }).catch(()=>{});
  },[]);
  ...
  const saveProfile = async ()=>{
    ...
    setProfile(updated);
    setEditing(false);
    await window.storage.set("kotonoha-profile", JSON.stringify(updated));
  };
...
function HirobaPage({ diaries, t }) {
  ...
  useEffect(() => {
    window.storage.get("hiroba-v3").then(r => {
      if (r) { const s = JSON.parse(r.value); setPlacedItems(s.p||[]); setInventory(s.i||[]); }
    }).catch(()=>{});
  }, []);

  const persist = (p, i) => window.storage.set("hiroba-v3", JSON.stringify({p,i})).catch(()=>{});
...
function AuthScreen({ onLogin, t }) {
  ...
  // Auth is handled in ProfilePage
  return (
    <div style={{
